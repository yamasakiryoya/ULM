...
dataname_set2 = [["abalone5","bank5'","bank5'","calhousing5","census5'","census5'","computer5'","computer5'"], 
                ["abalone10","bank10'","bank10'","calhousing10","census10'","census10'","computer10'","computer10'"], 
                ["car","ERA","LEV","SWD","winequality-red"]]
    ...
    for a in [0,1,2]:
        for b in [0,1,2,3,4,5,6,7]:
            if a!=2 or b<=4:
                ...
                name2 = dataname_set2[a][b]
                fig=plt.figure(figsize=(18,9)); ax = fig.add_subplot(111); plt.xscale('log')
                for method in method_set:
                    ...
                    for c in [0,1,2,3,4,5]:
                        ...
                        if os.path.isfile("./Results/%s/%s/%s-%d.csv"%(method,task,name,size)):
                            ...
                            if task=='S': ME[c], ST[c] = np.mean(np.power(data[:100,11],2)), np.std(np.power(data[:100,11],2))#data[100,11], data[101,11]
                    plt.plot(np.array(size_set), ME, lw=1, color=cmap((method_set.index(method)+2)/(len(method_set)-1+2)))
                    plt.errorbar(np.array(size_set), ME, yerr=ST, color=cmap((method_set.index(method))/(len(method_set)-1+1)), lw=1, capthick=1, label=method, capsize=10)
                plt.title("%s"%name2)
                if task=='P': plt.ylabel("NLL")
                else: plt.ylabel("M%sE"%task)
