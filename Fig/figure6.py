...
ax1.set_xlabel(r'$u$')
ax1.set_ylabel(r'$(P_{\rm acl}(y;\{\bm{b}-u\cdot\bm{1}\}/s))_{y\in[%d]}$'%cla)
ax1.set_title(r'$\bm{b}=(0,1,2,3,4,5,6,7,8), s=%s$'%s)
ax1.grid()
...
ax2.set_xlabel(r'$u$')
ax2.set_title(r'$\bm{b}=(0,1,2,3,4,5,6,7,8), s=%s$'%s)
ax2.grid()
...
ax3.set_xlabel(r'$u$')
ax3.set_title(r'$\bm{b}=(0,1,2,3,4,5,6,7,8), s=%s$'%s)
ax3.grid()
...
plt.tight_layout()
plt.savefig("./HS-ACL_%d.png"%cla,bbox_inches="tight", pad_inches=0.01,facecolor=fig.get_facecolor(), edgecolor='none')
plt.close()
